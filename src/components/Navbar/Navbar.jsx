...
//         className="text-4xl md:text-3xl font-extrabold tracking-wide bg-gradient-to-r from-pink-400 via-purple-400 to-blue-400 text-transparent bg-clip-text drop-shadow-lg"
//       >
//         Mansi's Matrix
//        </motion.span>
...
//       <div
//         className={`absolute top-20 right-6 w-3/4 max-w-xs bg-[#0f1a2a] bg-opacity-95 backdrop-blur-md shadow-xl rounded-xl p-6 transition-all duration-500 ${menu ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'
//           } md:hidden`}
//       >
  ...
  return (
    <nav className="relative z-50 w-full px-6 md:px-20 py-6 bg-gradient-to-r from-[#0f2027] via-[#203a43] to-[#2c5364] shadow-md text-white flex justify-between items-center">
      ...
      <motion.span
        ...
        animate={{ opacity: 1, scale: 1 }}
        transition={{ duration: 0.8, ease: "easeOut" }}
        className="text-4xl md:text-3xl font-extrabold tracking-wide bg-gradient-to-r from-white to- text-transparent bg-clip-text drop-shadow-lg"
      >
        Mansi's Matrix
      </motion.span>
      ...
      >
        <ul className="flex flex-col gap-4 text-lg font-semibold text-center">
          <li
            onClick={() => setMenu(false)}
            className="hover:text-[#94b4fa] transition-all"
          >
            <a href="#About">About</a>
          </li>
          <li
